...
TEST_CASE("Create an allocator from a memory resource") {
  ...
  REQUIRE_NOTHROW([](auto mem) {
    monotonic_allocator<int> monotonic{mem};
  }(&memoryResource));
}
...
TEST_CASE(
    "Allocate a struct aligned to 4 bytes, then one aligned to 8 bytes. Size "
    "of pool should be 16. Next allocation of any size should fail.") {
...
TEST_CASE(
    "Two allocators of the same type with the same memory resource should "
    "compare equal") {
  monotonic_memory memoryResource{16};
...
TEST_CASE(
    "Two allocators of the same type referencing different memory resources "
